...
namespace QuanLyHopDongVaKySo_API.Services.PFXCertificateService
{
    public interface IPFXCertificateSvc
    {
        ...
        Task<string> SignContract(string imagePath, string? imagePathStamp, string inputPdfPath, string outputPdfPath, string serialCerti, float xCoordinate, float yCoodinate,string typeDoc);
    }
}
